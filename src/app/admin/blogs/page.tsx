...
import siteContentConfig from '@/config/site-content.json'
...
import { BlogCoverHoverPreview, useBlogCoverHover } from '@/app/blog/components/blog-cover-hover'
...
export default function AdminBlogsPage() {
	...
	const enableCategories = siteContentConfig.enableCategories ?? false
	...
	return (
		<>
			...
			<div className='space-y-6'>
				<Card className='static p-4' staticLayout>
					<div className='flex flex-wrap items-center justify-between gap-3'>
						<div>
							...
							<p className='text-secondary mt-1 text-sm'>编辑文章分类、批量删除、隐藏文章。点击文章右侧的"编辑"跳转到写文章页面进行编辑。</p>
						</div>
						<Link href='/admin/blogs/write' className='brand-btn px-4 py-2 text-sm'>
							写新文章
						</Link>
					</div>
				</Card>
				...
				<Card className='static p-4' staticLayout>
					<div className='flex flex-wrap items-center justify-between gap-3'>
						<div className='card btn-rounded relative flex items-center gap-1 p-1 max-sm:hidden'>
							{[
								{ value: 'day', label: '日' },
								{ value: 'week', label: '周' },
								{ value: 'month', label: '月' },
						...
						<div className='flex items-center gap-2'>
							{editMode ? (
								...
								<button type='button' onClick={toggleEditMode} className='bg-card rounded-xl border px-6 py-2 text-sm backdrop-blur-sm transition-colors hover:bg-white/80'>
									编辑
								</button>
							)}
						</div>
				...
				<div className='space-y-4' onMouseLeave={cancelCoverPreview}>
					{groupKeys.map(groupKey => {
						const group = groupedItems[groupKey]
						if (!group) return null

						return (
							<Card key={groupKey} className='static space-y-4 p-5' staticLayout>
								...
								<div>
									{group.items.map(it => {
										...
										const linkHref = editMode ? '#' : `/blog/${it.slug}`
										return (
											<div
												...
												className={cn(
													...
													editMode
														? cn(
																'cursor-pointer rounded-lg border px-3',
												...
												<Link
													...
													className={cn('flex-1 truncate text-sm font-medium transition-all', editMode ? null : 'group-hover:text-brand group-hover:translate-x-2')}>
													{it.title || it.slug}
												</Link>
												{!editMode && (
													<Link
