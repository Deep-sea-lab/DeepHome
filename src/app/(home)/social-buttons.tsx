		...
		if (button.type === 'github') {
			return (
				<motion.a
					...
					target='_blank'
					{...commonProps}
					className={`font-averia flex items-center gap-2 overflow-hidden rounded-xl bg-[#070707] text-xl text-white ${!hasLabel ? 'p-1.5' : 'px-3 py-1.5'}`}>
	...
	return (
		<HomeDraggableLayer cardKey='socialButtons' x={x} y={y} width={styles.width} height={styles.height}>
			<motion.div className='absolute max-sm:static' animate={{ left: x, top: y }} initial={{ left: x, top: y }}>
				<div className='absolute top-0 left-0 flex flex-row-reverse items-center gap-3 max-sm:static max-sm:flex-row max-sm:justify-center' style={{ width: styles.width }}>
